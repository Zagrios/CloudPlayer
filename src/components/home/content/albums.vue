<template>
	<div id="artists-page">
        ...
        <playlistList :playlists="albums" @clickDeleteButton="deletePlaylist" @openFolder="openPlaylist" @download="downloadPlaylist"/>
    </div>
</template>
...
export default {
    ...
    methods:{
        deletePlaylist:function(data){
            this.EventBus.emit('openModal', {type: 'deleteTracks', parms:{'playlist': data, 'type':'album'}});
        },
        openPlaylist:function(albums){
            this.$router.push("/home/albums/"+albums.name);
        },
	...
	created(){
		...
		this.$store.commit('setAlbums', this.$func.asignAlbum(this.$store.getters.getTracks)); 
